...
func SearchProfile() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if profile != nil {
			c.JSON(http.StatusOK, responses.ProfileResponse{Status: http.StatusOK, Message: "Successfully found the  the document", Profile: s})
		} else {
			c.JSON(http.StatusInternalServerError, responses.ProfileResponse{Status: http.StatusInternalServerError, Message: "Error, Document Not found with the search query specified", Profile: s})
		}
