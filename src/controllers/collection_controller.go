...
var cluster *gocb.Cluster = db.InitializeCluster()
var scope *gocb.Scope = db.GetScope(cluster)
...
func insertDocument(context *gin.Context, collectionName string, docKey string, data interface{}) {
	...
	_, err := scope.Collection(collectionName).Insert(docKey, data, nil)
	if err != nil {
	...
	getResult, err := scope.Collection(collectionName).Get(docKey, nil)
	if err != nil {
		context.JSON(http.StatusInternalServerError, responses.TravelSampleResponse{
			Status:         http.StatusInternalServerError,
			Message:        "Internal Server Error",
...
func updateDocument(context *gin.Context, collectionName string, docKey string, data interface{}) {
	var getDoc interface{}
	_, err := scope.Collection(collectionName).Upsert(docKey, data, nil)
	if err != nil {
		context.JSON(http.StatusInternalServerError, responses.TravelSampleResponse{
			Status:         http.StatusInternalServerError,
			Message:        "Internal Server Error",
...
func deleteDocument(context *gin.Context, collectionName string, docKey string) {
	_, err := scope.Collection(collectionName).Remove(docKey, nil)
	if err != nil {
		if errors.Is(err, gocb.ErrDocumentNotFound) {
